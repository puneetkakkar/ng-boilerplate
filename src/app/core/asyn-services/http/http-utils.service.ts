...
function createQuery(pQuery: any, args: Array<any>): HttpParams {
  ...
  const query = typeof args !== 'undefined' ? args[0] : {};
  if (pQueryKey) {
...
export function methodBuilder(method: string) {
  return (url: string) => {
    return (target: HttpService, propertyKey: string, descriptor: any) => {
      ...
      const pQuery = (target as any)[`${propertyKey}_Query_parameters`];
      const pBody = (target as any)[`${propertyKey}_Body_parameters`];
      const pHeader = (target as any)[`${propertyKey}_Header_parameters`];
      const pResponseType = (target as any)[`${propertyKey}_ResponseType_parameters`];
